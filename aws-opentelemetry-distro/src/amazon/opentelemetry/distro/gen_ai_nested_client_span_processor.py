...
class _ShadowSpanProcessor(SpanProcessor):
    def __init__(
        self,
        processor: "GenAiNestedClientSpanProcessor",
        original_processor: SpanProcessor,
        original_context: SpanContext,
    ):
        self._processor = processor
        self._original_processor = original_processor
        self._original_context = original_context
        self._is_shadow_span = False

    def restore_context(self, span: Span) -> None:
        span._context = self._original_context  # noqa: SLF001

    def _on_ending(self, span: Span) -> None:
        self._is_shadow_span = not self._processor._is_allowlisted_gen_ai_span(span)  # noqa: SLF001
        if self._is_shadow_span:
        ...
        self.restore_context(span)
        self._original_processor._on_ending(span)  # noqa: SLF001

    def on_end(self, span: ReadableSpan) -> None:
        child_span_id = self._original_context.span_id
        if self._is_shadow_span:
            self._processor._on_suppressed_client_end(span, child_span_id)  # noqa: SLF001
            return

        self._processor._parent_spans.pop(child_span_id)  # noqa: SLF001
        self._original_processor.on_end(span)
...
class GenAiNestedClientSpanProcessor(SpanProcessor):
    ...
    def on_start(self, span: Span, parent_context=None) -> None:
        ...
        if isinstance(parent_span._span_processor, _ShadowSpanProcessor):  # noqa: SLF001
            parent_span._span_processor.restore_context(parent_span)  # noqa: SLF001
        ...
        span._span_processor = _ShadowSpanProcessor(  # noqa: SLF001
            self,
            span._span_processor,  # noqa: SLF001
            original_context,
        )
    ...
    def _on_suppressed_client_end(self, span: ReadableSpan, child_span_id: int) -> None:
